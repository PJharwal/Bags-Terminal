  ...
  return (
    <div className="w-full">
      <button
        ...
        className="btn-ghost flex items-center justify-center gap-1 w-full py-2 text-[10px] font-bold uppercase disabled:opacity-30 disabled:cursor-not-allowed"
      >
        {isProcessing ? (
          ...
          <XCircle size={10} className="text-[#666]" />
        ) : (
      ...
      {expanded && state !== 'idle' && (
        <div className="mt-2 p-3 border border-white/10 bg-[#0A0A0A] text-[10px] font-mono space-y-2">
          {state === 'checking' && (
          ...
          {state === 'available' && (
            <>
              ...
              <button
                ...
                className="btn-primary w-full py-1.5 text-[10px] font-bold uppercase"
              >
                Confirm & Pay
              </button>
            </>
          ...
          {state === 'unavailable' && (
            <p className="text-[#666]">{unavailableReason}</p>
          )}
          ...
          {state === 'signing' && (
            <div className="flex items-center gap-2 text-[#EDEDED]">
              <Loader2 size={10} className="animate-spin text-[#39FF14]" />
          ...
          {state === 'success' && (
            <div className="space-y-2">
              <p className="text-[#39FF14]">Listing submitted successfully.</p>
              <p className="text-[#666]">Your token info will appear on Dexscreener shortly.</p>
            </div>
          )}
          ...
          {state === 'error' && (
            <div className="space-y-2">
              ...
              <button
                onClick={reset}
                className="btn-ghost w-full py-1.5 text-[10px] font-bold uppercase"
              >
                Try Again
              </button>
            </div>
