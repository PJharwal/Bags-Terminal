...
function TrendIcon({ trend }: { trend: 'Improving' | 'Stable' | 'Deteriorating' }) {
    if (trend === 'Improving') return <TrendingUp size={12} className="text-acid-green" />;
    if (trend === 'Deteriorating') return <TrendingDown size={12} className="text-error" />;
    return <Minus size={12} className="text-fg-soft" />;
}
...
function PatternRow({ pattern }: { pattern: PatternFlag }) {
    ...
    return (
        ...
        >
            ...
            <span className="text-meta text-[#AAAAAA] group-hover:text-fg transition-colors">
                {pattern.explanation}
            </span>
        </div>
    );
...
function ScoreRow({
    ...
    return (
        <div className="flex items-start gap-3 py-2">
            <Icon size={14} className="text-muted-high mt-0.5 flex-shrink-0" />
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                    <span className="text-meta text-fg-soft uppercase tracking-wider">{label}</span>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-meta text-muted-high">{scoreLabel}</span>
                    </div>
                </div>
                <p className="text-meta text-[#AAAAAA]">{summary}</p>
            </div>
        </div>
    );
}
    ...
    if (!matrix) {
        return (
            <div className={`${layout === 'terminal' ? 'p-4' : 'p-3'} card`}>
                <div className="flex items-center gap-2 text-muted-mid text-xs">
                    <Shield size={14} />
        ...
        return (
            <div className="card p-3">
                ...
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        <Shield size={12} className="text-muted-mid" />
    ...
    return (
        <div className="card">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.06]">
                <div className="flex items-center gap-2">
                    <Shield size={14} className="text-acid-green" />
            ...
            <div className="px-4 py-2 border-b border-white/10">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-meta text-muted-high">{matrix.confidenceBand.trend}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="px-4 py-2">
                ...
                <div className="space-y-0.5">
                    {matrix.behaviorPatterns.length > 0 ? (
                        ...
                        <div className="text-meta text-[#444] py-1">No significant patterns detected</div>
                    )}
                </div>
            </div>
        </div>
