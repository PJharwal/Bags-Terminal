  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-fg uppercase tracking-widest text-display">Fee Share</h2>
        <div className="flex items-center gap-2">
          <span className="text-meta text-muted-high font-mono">
            {feeClaimers.length}/{MAX_FEE_CLAIMERS}
          </span>
          <span className={`text-meta font-mono ${isValid ? 'text-acid-green' : 'text-error'}`}>
            {totalPercentage}%
          </span>
        </div>
      </div>
      ...
      {!isValid && feeClaimers.length > 0 && (
        <div className="badge-red flex items-center gap-2 p-2.5 text-meta">
      ...
      {needsLookupTable && (
        <div className="badge-blue flex items-center gap-2 p-2.5 text-meta">
          <Info size={12} />
          <span className="font-mono">
          ...
          {feeClaimers.map((claimer) => (
            ...
            >
              <div className="flex flex-col">
                ...
                <span className="text-meta text-muted-high uppercase">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 text-muted-high hover:text-error hover:bg-[#FF003C]/10 transition-all"
                >
                  <Trash2 size={12} />
                </button>
              </div>
      ...
      {showAddForm ? (
        ...
        <button
          ...
          disabled={isAtMax}
          className="btn-ghost btn-press flex items-center justify-center gap-2 py-2.5 border-dashed text-meta font-bold uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed"
        >
          <Plus size={12} />
          Add Fee Claimer
        </button>
      ...
      {feeClaimers.length === 0 && (
        <p className="text-meta text-muted-high font-mono">
          Add up to {MAX_FEE_CLAIMERS} fee claimers to receive trading fees from your token.
        </p>
      )}

      {/* No API fees notice */}
      <div className="flex items-center gap-2 p-2 bg-acid-green/5 border border-[#39FF14]/20">
        <Info size={10} className="text-acid-green shrink-0" />
        <span className="text-meta text-acid-green/70 font-mono">
