...
// Shared 1s ticker — one interval for all cards, driven via useSyncExternalStore,
// so the displayed age stays live instead of freezing at ingestion time.
let nowSecond = Math.floor(Date.now() / 1000);
const nowListeners = new Set<() => void>();
let nowInterval: ReturnType<typeof setInterval> | null = null;
function subscribeNowSecond(listener: () => void) {
...
const getNowSecond = () => nowSecond;
function useNowSecond() {
  return useSyncExternalStore(subscribeNowSecond, getNowSecond, getNowSecond);
}
...
const getRingColor = (riskFlags: PulseItem["riskFlags"]): string => {
  if (riskFlags.some((f) => f.severity === "critical")) return "#ef4444";
  if (riskFlags.some((f) => f.severity === "warn")) return "#fbbf24";
  return "#2a2a35";
};
          ...
          <div className="flex flex-col min-w-0 pr-2">
            ...
            <div className="flex items-center gap-1 text-[9.8px] text-[#777a8c] mt-[1px] overflow-hidden">
              <span className="text-[#16a34a] shrink-0 min-w-[20px]">
                {formatTimeAgo(
                  token.createdAtSec != null
                    ? Math.max(0, now - token.createdAtSec)
                    : token.ageSeconds,
                )}
              </span>
              {token.holders > 0 && (
