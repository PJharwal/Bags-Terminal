...
export function QuickBuyProvider({ children }: { children: ReactNode }) {
  ...
  // clear the in-flight buy instead of spinning forever.
  ...
  }, [lastSignature, connection, setBuy, clearWatchdog]);
  ...
  useEffect(() => {
    ...
    setBuy(null);
  }, [lastError, setBuy, clearWatchdog]);
  ...
  const quickBuy = useCallback(
    async (mint: string, solAmount: number, symbol?: string) => {
      ...
      try {
        ...
        if (!instantBuy({ mint, solAmount, ...hint })) {
          toast.error("Buy not submitted — re-authenticate or reconnect");
          setBuy(null);
          return;
        }
        clearWatchdog();
