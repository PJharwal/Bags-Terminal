...
export function WalletButton() {
  ...
  if (connecting) {
    return (
      <button className="px-4 py-1.5 bg-transparent border border-[#39FF14]/50 text-[#39FF14] text-[10px] font-bold tracking-widest animate-pulse" disabled>
        CONNECTING...
      </button>
    );
  }
        ...
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-[#0A0A0A] border border-white/10 z-50 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm font-bold uppercase tracking-wider text-[#EDEDED] flex items-center gap-2">
              <User size={14} className="text-[#39FF14]" /> Wallet Profile
            </h2>
            <button onClick={closeProfile} className="text-[#666] hover:text-[#EDEDED] text-xs">CLOSE</button>
          </div>

          {/* Phantom Wallet */}
          <div className="mb-4 p-3 bg-[#1A1A1A] border border-white/10">
            <div className="text-[9px] text-[#666] uppercase tracking-widest mb-2">Identity Wallet (Phantom)</div>
            <div className="flex items-center justify-between gap-2">
              <code className="text-[10px] font-mono text-[#EDEDED] break-all">{publicKey}</code>
              <button onClick={() => handleCopy('phantom', publicKey!)} className="p-1 hover:bg-white/10 flex-shrink-0">
                {copied === 'phantom' ? <span className="text-[9px] text-[#39FF14]">Copied!</span> : <Copy size={12} className="text-[#666]" />}
              </button>
            </div>
            {balance !== null && <div className="text-[10px] text-[#666] font-mono mt-1">Balance: {balance.toFixed(4)} SOL</div>}
          </div>

          {/* Turnkey Trading Wallet */}
          {isAuthenticated && turnkeyAddress ? (
            <div className="mb-4 p-3 bg-[#1A1A1A] border border-[#39FF14]/20">
              <div className="flex items-center justify-between mb-2">
                <div className="text-[9px] text-[#666] uppercase tracking-widest flex items-center gap-1">
                  <Zap size={10} className="text-[#39FF14]" /> Trading Wallet (Turnkey)
                </div>
                <span className="text-[9px] text-[#39FF14] font-mono">Active</span>
              </div>
              <div className="flex items-center justify-between gap-2">
                <code className="text-[10px] font-mono text-[#EDEDED] break-all">{turnkeyAddress}</code>
                <button onClick={() => handleCopy('turnkey', turnkeyAddress)} className="p-1 hover:bg-white/10 flex-shrink-0">
                  {copied === 'turnkey' ? <span className="text-[9px] text-[#39FF14]">Copied!</span> : <Copy size={12} className="text-[#666]" />}
                </button>
              </div>
              {turnkeyBalance !== null && <div className="text-[10px] text-[#666] font-mono mt-1">Balance: {turnkeyBalance.toFixed(4)} SOL</div>}
              <a href={`https://solscan.io/account/${turnkeyAddress}`} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1 mt-2 text-[10px] text-[#888] hover:text-[#EDEDED] transition-colors">
                <ExternalLink size={10} /> View on Solscan
              </a>
              {(turnkeyBalance === null || turnkeyBalance === 0) && (
                <div className="mt-3 p-2 bg-[#FF003C]/10 border border-[#FF003C]/30 text-[9px] text-[#FF003C] font-mono">
                  This wallet has no SOL. Send SOL to the address above to start trading.
                </div>
              )}
            </div>
          ) : (
            <div className="mb-4 p-3 bg-[#1A1A1A] border border-white/10 text-[10px] text-[#666]">
            ...
            <div className="mb-4">
              {exportStep === 'idle' && (
                <button onClick={() => setExportStep('warning')}
                  className="w-full py-2.5 text-[10px] font-bold uppercase tracking-wider border border-[#FF003C]/30 text-[#FF003C] hover:bg-[#FF003C]/10 transition-colors flex items-center justify-center gap-2">
                  <Key size={12} /> Export Private Key
                </button>
              )}

              {exportStep === 'warning' && (
                <div className="p-3 bg-[#1A1A1A] border border-[#FF003C]/30">
                  <div className="flex items-start gap-2 mb-3">
                    <AlertTriangle size={14} className="text-[#FF003C] flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-[10px] font-bold text-[#FF003C] mb-1">Warning: Extremely sensitive!</p>
                      <ul className="text-[9px] text-[#FF003C]/80 space-y-0.5">
                  ...
                  {exportError && <p className="text-[9px] text-[#FF003C] mb-2 font-mono">{exportError}</p>}
                  <div className="flex gap-2">
                    <button onClick={() => { setExportStep('idle'); setExportError(null); }}
                      className="flex-1 py-2 text-[10px] border border-white/10 text-[#888] hover:bg-white/5">Cancel</button>
                    <button onClick={handleExport}
                      className="flex-1 py-2 text-[10px] bg-[#FF003C] text-white font-bold hover:brightness-110">I Understand, Export</button>
                  </div>
                </div>
              )}

              {exportStep === 'loading' && (
                <div className="p-4 bg-[#1A1A1A] border border-white/10 flex flex-col items-center gap-2">
                  <Loader2 size={20} className="text-[#39FF14] animate-spin" />
                  <p className="text-[10px] text-[#888]">Exporting and decrypting...</p>
                </div>
              )}

              {exportStep === 'result' && exportedKey && (
                <div className="p-3 bg-[#1A1A1A] border border-[#FF003C]/30">
                  <div className="text-[9px] text-[#FF003C] uppercase tracking-widest mb-2 flex items-center gap-1">
                    <Key size={10} /> Your Private Key
                  </div>
                  <div className="relative">
                    <code className="block text-[10px] font-mono text-[#EDEDED] break-all bg-black/50 p-2 pr-8">{exportedKey}</code>
                    <button onClick={() => handleCopy('key', exportedKey)} className="absolute top-1 right-1 p-1 hover:bg-white/10">
                      {copied === 'key' ? <span className="text-[8px] text-[#39FF14]">Copied!</span> : <Copy size={10} className="text-[#666]" />}
                    </button>
                  </div>
                  <p className="text-[9px] text-[#FF003C]/70 mt-2">Store this securely. Never share it with anyone.</p>
                  <button onClick={() => { setExportStep('idle'); setExportedKey(null); }}
                    className="mt-2 text-[9px] text-[#888] hover:text-[#EDEDED]">Hide Key</button>
                </div>
              )}
            </div>
          )}
          ...
          <button onClick={handleDisconnect}
            className="w-full py-2.5 text-[10px] font-bold uppercase tracking-wider border border-white/10 text-[#FF003C] hover:bg-[#FF003C]/10 transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setDropdownOpen(!dropdownOpen)}
        ...
        {turnkeyBalance !== null && <span className="text-[#888] font-mono">{turnkeyBalance.toFixed(2)} SOL</span>}
      ...
      {dropdownOpen && (
        <div className="dropdown-enter absolute right-0 top-full mt-1 w-56 bg-[#0A0A0A] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] z-50">
          <div className="px-3 py-2 border-b border-white/5">
            <div className="text-[8px] text-[#666] uppercase tracking-widest">Phantom</div>
            <div className="text-[10px] font-mono text-[#888]">{shortenedAddress}</div>
          </div>
          {isAuthenticated && turnkeyAddress && (
            <div className="px-3 py-2 border-b border-white/5">
              <div className="text-[8px] text-[#39FF14] uppercase tracking-widest flex items-center gap-1"><Zap size={8} /> Trading Wallet</div>
              <div className="text-[10px] font-mono text-[#EDEDED]">{turnkeyAddress.slice(0, 4)}...{turnkeyAddress.slice(-4)}</div>
              <div className="text-[9px] font-mono text-[#666]">{turnkeyBalance !== null ? `${turnkeyBalance.toFixed(4)} SOL` : '--'}</div>
            </div>
          )}
